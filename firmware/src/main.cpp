...
// On the ESP32-S3-BOX the only available physical inputs are GPIO0 (BOOT)
// and GPIO1 (mute slider switch). We can't replicate the Waveshare's
// 3-button layout, so BTN_BACK maps to BOOT (cycle screen / splash next)
// and BTN_FWD maps to the mute switch (Shift+Tab toggle).
#define BTN_BACK 0
#define BTN_FWD  1

Arduino_DataBus *bus = new Arduino_ESP32SPI(
    LCD_DC, LCD_CS, LCD_SCLK, LCD_MOSI, -1 /* MISO */, FSPI);
// ESP32-S3-BOX uses ILI9342C (native landscape, IPS, inverted colors).
...
static void touch_read() {
#if defined(BOARD_S3BOX)
    // Poll the TT21100 directly at ~20 Hz. The INT line on this board variant
    // doesn't fire reliably, so we just ask the controller for a frame on a
    // timer; reads with data_len < 8 cost ~2 I2C bytes and are cheap.
    ...
    if (tt21100_read(&tx, &ty)) {
        ...
        // No touch frame for ~80ms — call it a release.
        touch_pressed = false;
    }
    return;
#endif
...
#if defined(BOARD_S3BOX)
    // ILI9342C/ILI9341 driver handles rotation natively; no CPU remap needed.
...
void setup() {
    ...
    // Init touch
#if defined(BOARD_S3BOX)
    // The TT21100 shares its RST line with the LCD; gfx->begin() pulsed it,
    // so wait for the controller to come back up before probing it.
    delay(150);
    if (!tt21100_init()) {
        Serial.println("Touch init failed");
    } else {
        pinMode(TP_INT, INPUT_PULLUP);
        // INT goes low while a touch frame is pending — read on either edge so
        // we also notice the "data still available" state after wake from reset.
...
#if defined(BOARD_S3BOX)
    // BOX has no IMU and the LCD driver doesn't expose setBrightness — the
    // panel is fixed-orientation and the backlight is a plain GPIO.
    return;
#else
...
void loop() {
    ...
    //
    // Waveshare (3 buttons): LEFT=Space, RIGHT=Shift+Tab, PWR=cycle screens.
    // S3-BOX (2 inputs): only GPIO0 (BOOT momentary) and GPIO1 (Mute slider)
    // are exposed, so the Space binding is dropped and BOOT doubles as the
    // screen-cycle button. The Mute switch sends Shift+Tab on each flip.
    {
        ...
        if (back_now != back_was) {
            if (back_now) {
                // Touch is unreliable on this panel — make BOOT always advance
                // the screen so the user can leave the splash without it. The
                // splash auto-rotates its animation every 20s on its own.
        ...
        if (fwd_now != fwd_was) {
            // Slider toggled — fire one Shift+Tab tap and immediately release.
